...
- 🔄 **自动更新**：安装版自动检查 GitHub Releases，支持下载进度和重启安装。
- 📁 **用户数据路径**：默认将工作空间和软件数据保存到安装目录下的 `LaunchPadData` 文件夹。
- 🟢 **运行状态更准确**：改进工作空间与软件进程识别，启动后状态能够及时刷新。
- 🌍 **中英日三语言**：界面、菜单、托盘与运行提示支持简体中文、English 和日本語。
- 🎨 **全新应用图标**：统一窗口、任务栏、托盘、安装包与 EXE 文件图标。

[查看 v1.4.1 完整更新日志](CHANGELOG.md#141---2026-08-12)
...
- **`LaunchPad-Setup-1.4.1-x64.exe`**：推荐，大多数用户选择此安装包；支持自选安装目录，并创建桌面与开始菜单快捷方式。
- **`LaunchPad-Portable-1.4.1-x64.exe`**：免安装便携版，双击即可运行。
